...
Enable monorepo support in `entkapp/config.json`:
...
If `ui-components` exports a `Button` that is only used in `web-app`, entkapp knows it's used. If it exports a `Slider` that is used nowhere in the entire monorepo, it will be flagged.

## Commands

### Analyze entire monorepo
```bash
npx entkapp --workspace -r
```
...
```bash
npx entkapp --package @my-org/ui-components
```
